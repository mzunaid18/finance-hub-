...
def dashboard():
    ...
    # Get only current user's transactions, newest first
    cursor.execute(
    ...
    cursor.execute(
        "SELECT category, SUM(amount) FROM transactions WHERE type='Expense' AND user_id=? GROUP BY category",
        (session["user_id"],),
    )
...
def add_transaction():
    ...
    if request.method == "POST":
        ...
        cursor.execute(
            """
            INSERT INTO transactions (date,category,type,amount,user_id)
            VALUES(?,?,?,?,?)
            """,
            (
                date,
...
def export_csv():
    ...
    cursor.execute(
        "SELECT date, category, type, amount FROM transactions WHERE user_id=?",
        (session["user_id"],),
    )
    transactions = cursor.fetchall()
...
def export_pdf():
    ...
    pdf.drawString(200, 750, "FinanceHub Transaction Report")
    ...
    for transaction in transactions:
        pdf.drawString(50, y, str(transaction[0]))
        pdf.drawString(150, y, str(transaction[1]))
        pdf.drawString(280, y, str(transaction[2]))
        pdf.drawString(380, y, "₹" + str(transaction[3]))
        y -= 25
        if y < 50:
            pdf.showPage()
            y = 750
...
@app.route("/log_calculation", methods=["POST"])
def log_calculation():
    ...
    try:
        cursor.execute(
            ...
            (user_id, calc_type, inputs, result, timestamp)
            VALUES (?, ?, ?, ?, datetime('now'))
            """,
            (
                user_id,
...
def gst():
    ...
    return render_template("gst.html", gst_amount=gst_amount, total_amount=total_amount)
